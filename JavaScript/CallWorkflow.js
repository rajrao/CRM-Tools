...
(function () {
    ...
    this.executeWorkflow = function (workflowId, entityId) {
        ...
        if (!isString(workflowId)) {
            throw new Error("ai.WebAPI.executeWorkflow workflowId parameter must be a string.");
        }
        if (!isString(entityId)) {
            throw new Error("ai.WebAPI.executeWorkflow entityId parameter must be a string.");
        }
